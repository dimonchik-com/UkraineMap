...
package  silin.geom
{
	...
	public class PathSegment 
	{
		...
		public function PathSegment(p0:Point, p1:Point, p2:Point) 
		{
			...
			var a2t:Number = _a*2;
			_length = (2 * _sqrt_a * (d * (_b + a2t) - _sqrt_c * _b) + (_b * _b - 4 * _a * _c) * (Math.log(2 * _sqrt_c + _b / _sqrt_a) - Math.log(2 * d + (_b + a2t) / _sqrt_a))) / (8 * Math.pow(_a, (3 / 2)));
		...
		public function getPoint(pos:Number):PathPoint
		{
			...
			while (Math.abs(targ - pos) > 1e-6 && max_i-->0)
			{
				...
				targ = (2 * _sqrt_a * (d * (_b + a2i) - _sqrt_c * _b) + (_b * _b - 4 * _a * _c) * (Math.log(2 * _sqrt_c + _b / _sqrt_a) - Math.log(2 * d + (_b + a2i) / _sqrt_a))) / (8 * Math.pow(_a, (3 / 2)));
				st /= 2;
			...
			var r:Number=Math.atan2(_p0.y - _p1.y + (2 * _p1.y - _p0.y - _p2.y) * f, _p0.x - _p1.x + (2 * _p1.x - _p0.x - _p2.x) * f) / (Math.PI / 180);
