...
package  silin.bitmap
{
	...
	public class BitmapTransform
	{
		 ...
		 * @return
		 */
		static public function getDominateBitmapColors(bitmapData:BitmapData, len:int = 8, graySort:Boolean=true):Array
		{
			...
			// собираем оcтавшиеся после загрубления цвета 
			var map:Object = {};
			for (var i:int = 0; i < bmd.width; i++) 
			{
				for (var j:int = 0; j < bmd.height; j++) 
				{
					map[bmd.getPixel(i, j)] = int(map[bmd.getPixel(i, j)]) + 1;;
				}
			}
		 ...
		 * @return
		 */
		public static function getAverageColor(src:BitmapData):int
		{
			...
			for (var i:int = 0; i < src.width; i++)
			{
				for (var j:int = 0; j < src.height; j++)
				{
			...
			var count:int = src.width * src.height;
			r /= count;
			g /= count;
			b /= count;
			return  r << 16 | g << 8 | b;
		...
		public static function bucketFill(bmd:BitmapData, x:int, y:int, color:int, tol:int = 64):void
		{
			...
			//битмап разеицы между оригиналом и цветом
		...
		public static function decolor(bmd:BitmapData, tol:int):void
		{
			...
			for (var i:int = 0; i < bmd.width; i++)
			{
				for (var j:int = 0; j < bmd.height; j++)
				{
					var clr:int = Color.round(bmd.getPixel(i, j), tol);
					bmd.setPixel(i, j, clr);
				}
			}
			bmd.unlock();
		}
		...
		/*
		//debug, не используется нигде
		public static function reduceGrayColor(bmd:BitmapData, tol:int):void
		{
		for (var i:int = 0; i < bmd.width; i++)
		{
		for (var j:int = 0; j < bmd.height; j++)
		{
		var clr:int = bmd.getPixel(i, j) & 0xFF;
		clr = tol * Math.round(clr / tol);
		bmd.setPixel(i, j, clr << 16 | clr << 8 | clr);
		}
		}
		}
		*/
